...
`#name Draw Voodoo Doll Closet (mine)`;
...
let numnewtags = 1;
let newtagindex = 0;
let ceilingHeight = UDB.ScriptOptions.triggertype == 0 ? 56 : 128;
let blockingCeilingHeight = UDB.ScriptOptions.triggertype == 0 ? 56 : 55;
let blockingFloorHeight = UDB.ScriptOptions.triggertype == 0 ? 1 : 0;

// We need an additional tag if the player is blocked at the beginning
if (UDB.ScriptOptions.inactive) numnewtags++;

// We need an additional tag if the closet should be looping
if (UDB.ScriptOptions.looping) numnewtags++;
...
p.setAngle(90 * UDB.ScriptOptions.direction)
  .moveTo(basepos)
  .drawVertex()
  .moveForward(UDB.ScriptOptions.length)
  .drawVertex()
  .turnRight()
  .moveForward(closetwidth)
  .drawVertex()
  .turnRight()
  .moveForward(UDB.ScriptOptions.length)
  .drawVertex();

if (!p.finishDrawing(true)) throw "Something went wrong while drawing!";
...
if (!p.finishDrawing()) throw "Something went wrong while drawing!";

// Assign the action and tag to the line
let carryline = UDB.Map.getMarkedLinedefs()[0];
carryline.action = 253;
...
if (UDB.ScriptOptions.inactive) {
  ...
  sector.tag = tags[newtagindex];
  sector.floorHeight = blockingFloorHeight;
  sector.ceilingHeight = blockingCeilingHeight;
  ...
  triggerlines.forEach((tl) => {
    if (
      tl.front.upperTexture.startsWith("SW1") ||
      tl.front.upperTexture.startsWith("SW2") ||
      tl.front.middleTexture.startsWith("SW1") ||
      tl.front.middleTexture.startsWith("SW2") ||
      tl.front.lowerTexture.startsWith("SW1") ||
      tl.front.lowerTexture.startsWith("SW2")
    ) {
...
if (UDB.ScriptOptions.looping) {
  ...
  p.setAngle(90 * UDB.ScriptOptions.direction)
    .moveTo(basepos)
    .moveForward(UDB.ScriptOptions.length - 32)
    .turnRight()
    .moveForward(8)
    .drawVertex()
    .moveForward(closetwidth - 16)
...
let newpos =
  new UDB.Vector2D(16, 16).getRotated(
    UDB.Angle2D.doomToReal(-90 * UDB.ScriptOptions.direction - 90),
  ) + basepos;
